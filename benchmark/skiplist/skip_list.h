...
template <typename T,
          ...
          typename LevelGenerator  = detail::skip_list_level_generator<256>,
...
public:
    ...
    insert_by_value_result insert(const value_type &value, int level = -1);
...
skip_list<T,C,A,LG,D>::insert(const value_type &value, int level)
{
    node_type *node = impl.insert(value, nullptr, level);
...
public:
    ...
    node_type       *insert(const value_type &value, node_type *hint = 0, int level = -1);
...
sl_impl<T,C,A,LG,AllowDuplicates>::insert(const value_type &value, node_type *hint, int level)
{
    if (level == -1) {
        level = new_level();
    } else {
